...
H --> I[Tabla Analítica + Order items]

C -->|merge_order_items| I

I --> J[Tabla Analítica + Products]

D -->|merge_order_products| J

J --> K[Tabla Analítica + Payments]

E -->|merge_order_payments| K

K --> L[Tabla Analítica + Sellers]

F -->|merge_sellers| L

L --> M[Tabla Analítica + Order Reviews]
...
- Se emplearon uniones `LEFT JOIN` para conservar todos los pedidos durante el proceso de integración.

- Cada unión utiliza el parámetro `validate` de `pandas.merge()` para comprobar que la cardinalidad esperada sea correcta.

- La tabla analítica se construye de forma incremental mediante funciones independientes, facilitando el mantenimiento y las pruebas del pipeline.

- Cada función incorpora una única dimensión de información antes de devolver el resultado para el siguiente paso del proceso.